...
QString AsemanCalendarConverter::convertIntToNumStringDate(qint64 d)
{
    ...
    date = date.addDays(d);
    return AsemanTools::translateNumbers( p->calendar->numberString(date) );
}

QString AsemanCalendarConverter::translateInt(qint64 d)
{
    return AsemanTools::translateNumbers(QString::number(d));
}
...
QString AsemanCalendarConverter::convertIntToStringDate(qint64 d, const QString &format)
{
    ...
    QDate date = QDate(1,1,1);
    date = date.addDays(d);
    return AsemanTools::translateNumbers( p->calendar->historyString(date) );
}
...
{
    return AsemanTools::translateNumbers( p->calendar->paperString(dt) );
}

QString AsemanCalendarConverter::convertDateTimeToString(const QDateTime &dt, const QString &format)
{
    return AsemanTools::translateNumbers( p->calendar->paperString(dt, format) );
}

QString AsemanCalendarConverter::convertDateTimeToLittleString(const QDate &dt)
{
    return AsemanTools::translateNumbers( p->calendar->littleString(dt) );
}
